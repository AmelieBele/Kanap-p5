...
// Récuperation et mise en forme du produit appeler
async function getProduits() {
  const listeProduits = await fetch(`http://localhost:3000/api/products/${id}`);
  const produitMisEnForme = await listeProduits.json();
  return produitMisEnForme;
}

// fonction main exécutée au chargement de la page
(async function main() {
  ...
  produitImage.setAttribute("alt", "Photographie d'un canapé");
  ...
  for (color of canape.colors) {
    conteneurProduitCouleurs.options[conteneurProduitCouleurs.options.length] =
  ...
  // Fonction d'ajout au panier au clic du bouton "ajouter au panier"
  ajouterAuPanierBouton.addEventListener("click", function () {
    ...
    const produitFound = cart.find(
      (produit) =>
        produit.id == produitPanier.id && produit.colors == produitPanier.colors
    );
